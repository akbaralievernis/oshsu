    ...
    <div className="min-h-screen transition-colors duration-300 dark:bg-slate-950 bg-slate-50 dark:text-white text-slate-900 font-sans flex flex-col lg:flex-row overflow-x-hidden">
      ...
      <header className="lg:hidden w-full flex items-center justify-between px-6 py-4 dark:bg-slate-900/80 bg-white border-b dark:border-slate-900 border-slate-200 backdrop-blur-lg sticky top-0 z-30 shadow-sm transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-lg dark:bg-slate-950 bg-slate-100 border dark:border-slate-850 border-slate-200 text-xs"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 bg-slate-950/60 backdrop-blur-md z-40 animate-fadeIn" onClick={() => setMobileMenuOpen(false)}>
          <aside 
            className="w-72 max-w-[80vw] h-full dark:bg-slate-900 bg-white p-6 border-r dark:border-slate-850 border-slate-200 flex flex-col justify-between"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="space-y-6">
              {/* Profile Card */}
              <div className="p-4 rounded-2xl dark:bg-slate-950 bg-slate-100 border dark:border-slate-850 border-slate-200 space-y-4">
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-650 dark:text-red-400 text-sm font-bold rounded-2xl border border-red-500/10 transition-all"
              >
      ...
      <aside className="hidden lg:flex w-80 dark:bg-slate-900/60 bg-white border-r dark:border-slate-900/80 border-slate-200 p-6 flex-col justify-between shrink-0 z-20 transition-colors duration-300">
        <div className="space-y-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-red-650 dark:text-red-500 font-bold tracking-widest uppercase">{d.student}</span>
            </div>
          </div>
            ...
            {hasRoom && (
              <div className="space-y-2 border-t dark:border-slate-900 border-slate-200 pt-3 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-red-600 font-extrabold">{assignedRoom.roomNumber} ({assignedRoom.floor}-кабат)</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-650 dark:text-red-400 text-sm font-bold rounded-2xl border border-red-500/10 transition-all duration-300"
          >
            <LogOut className="w-4 h-4" />
            {d.logout}
          </button>
          ...
          <div className="lg:col-span-2 space-y-8">
            {hasRoom ? (
              <div className="p-6 md:p-8 rounded-3xl dark:bg-slate-900/30 bg-white border dark:border-slate-900 border-slate-200 shadow-md space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-2xl dark:bg-slate-950 bg-slate-100 border dark:border-slate-900 border-slate-200">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-sm font-extrabold text-red-600">{assignedRoom.bedNumber} (Бөлмө {assignedRoom.roomNumber})</div>
                  </div>
                </div>
                  ...
                  <div className="divide-y dark:divide-slate-900 divide-slate-200">
                    {assignedRoom.roommates.map((mate, idx) => (
                      <div key={idx} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
                        <div className="w-9 h-9 rounded-xl dark:bg-slate-800 bg-slate-200 flex items-center justify-center border dark:border-slate-700 border-slate-300 text-xs font-bold text-red-600 shrink-0">
              ...
              <div className="p-8 rounded-3xl dark:bg-slate-900/30 bg-white border dark:border-slate-900 border-slate-200 shadow-md space-y-6 text-center py-12">
                <FileText className="w-12 h-12 text-red-600 mx-auto opacity-70" />
          ...
          <div className="space-y-8">
            ...
            <div className="p-6 md:p-8 rounded-3xl dark:bg-slate-900/30 bg-white border dark:border-slate-900 border-slate-200 shadow-md space-y-6">
              ...
              <div className="p-6 rounded-2xl dark:bg-slate-950 bg-slate-100 border dark:border-slate-900 border-slate-200 space-y-4">
                ...
                <button className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-red-600 to-rose-700 hover:from-red-750 hover:to-rose-800 text-white font-bold rounded-xl transition-all duration-300">
                  {d.btnPay}
                </button>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmitTicket} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full dark:bg-slate-950 bg-white border dark:border-slate-900 border-slate-200 dark:text-white text-slate-900 rounded-xl py-3 px-4 placeholder-slate-400 dark:placeholder-slate-700 text-xs focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 transition-all animate-fadeIn"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full dark:bg-slate-950 bg-white border dark:border-slate-900 border-slate-200 dark:text-white text-slate-900 rounded-xl py-3 px-4 text-xs focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 transition-all"
                  >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 dark:bg-slate-950 bg-slate-100 hover:bg-slate-200 dark:hover:bg-slate-900 border dark:border-slate-800 border-slate-200 text-xs font-bold rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {submittingTicket ? (
